...
felinos.forEach(function (felino) {
    ...
    let h2 = document.createTextNode(felino.titulo)
    let p = document.createTextNode(felino.paragrafo)

    titulo.appendChild(h2)
    paragrafo.appendChild(p)
    imagem.setAttribute('src', felino.imagem)
    item.appendChild(imagem)
    item.appendChild(titulo)
    item.appendChild(paragrafo)
    item.classList.add('item')

    document.querySelector('.felinos').appendChild(item)
})